...
function form_submitted($contact_form) {
  ...
  elseif (isset($user_id_array)) {
    write_log('didnt find user by email, try finding him by fingerprint');
    if (count($user_id_array) >= 2) { $user_id = merge_2_users($user_id_array[0], $user_id_array[1]); }
    ...
    $user_id = create_pipedrive_user('{"name": "' . $name . '", "email": "' . $email . '" "visible_to": "3", "26dccb2d4b7b701a77f418266af26599c970a414":"' . $_SESSION["fingerprint_session"] . '"}');
  }
  ...
  //a few other random things also break it, remove them
